...
const automationJobCurrent = document.querySelector("#automation-job-current");
...
const llmPanelContent = document.querySelector("#llm-panel-content");
...
const parseStructuredLinks = (value) => {
  if (Array.isArray(value)) {
    return value
      .map((item) => ({
        platform: compact(item?.platform || "other") || "other",
  ...
  if (typeof value === "string") {
    ...
    try {
      ...
      return [{ platform: "other", url: normalized, title: "" }];
    }
  }
  return [];
};
  ...
  if (editor) {
    editor.innerHTML = normalized.collectionLinks.length
      ? normalized.collectionLinks
          .map(
            (link, index) =>
              `<button type="button" class="owner-link-chip" data-info-panel-link-edit="${index}" title="${escapeHtml(link.title || link.url)}">${escapeHtml(
...
const openInfoPanelTextDialog = (form) => {
  ...
  textDialogTitle.textContent = "编辑相关导览文本";
...
const getEntityAbbreviations = (entity) =>
  (entity?.aliases || []).filter((value) => /^[A-Z0-9][A-Z0-9 .&/-]{1,15}$/.test(compact(value)));
const getDisplayTitle = (entity) =>
  (entity?.aliases || []).find((value) => /[\u3400-\u9fff]/.test(value) && compact(value).length < compact(entity?.name).length) ||
  entity?.name ||
  "";
const getDisplaySecondary = (entity) =>
  [
    entity?.name && entity?.name !== getDisplayTitle(entity) ? entity.name : "",
    entity?.nameLatin || "",
  ]
    .filter(Boolean)
    .join(" / ");
const buildOptionLabel = (entity) => {
  const title = getDisplayTitle(entity);
  const secondary = getDisplaySecondary(entity);
...
const normalizeListValue = (value) => {
  ...
  if (typeof value === "string") {
    return parseLines(value, (line) => line);
  }
  return [];
};
...
const renderIssues = () => {
  ...
  issuesPanel.innerHTML =
    state.dataIssues
      ...
      .map(
        (issue) => `
          <article class="owner-issue-item">
...
const refreshOverview = () => {
  ...
  const stats = [
    ["作曲家", state.library.composers.length],
  ...
  const composers = [...state.library.composers].sort((a, b) => buildComposerOptionLabel(a).localeCompare(buildComposerOptionLabel(b), "zh-Hans-CN"));
...
const fillNamedEntityForm = (form, entity) => {
  ...
  form.elements.country.value = entity?.country || "";
...
const buildWorkPayload = (form) => ({
  ...
  titleLatin: compact(form.elements.titleLatin.value),
  catalogue: compact(form.elements.catalogue.value),
  groupPath: compact(form.elements.groupPath.value)
    .split("/")
    .map((item) => item.trim())
    .filter(Boolean),
  slug:
...
const getStructuredLinkMode = (host) => (host?.matches?.("[data-info-panel-link-host]") ? "info-panel" : "recording");
...
const getInfoPanelLinksField = (host) =>
  host?.elements?.infoPanelCollectionLinks || host?.querySelector?.('[name="infoPanelCollectionLinks"]');
...
const openRecordingLinkDialog = (host, index) => {
  ...
  const link = links[index] || { platform: "", url: "", title: "" };
  const mode = getStructuredLinkMode(host);
  state.recordingLinkDialogContext = { form: host, index, mode };
  linkDialogTitle.textContent =
    index >= 0
      ? `${mode === "info-panel" ? "编辑合集链接" : "编辑资源链接"}：${buildRecordingLinkChipLabel(link, index, links)}`
      : mode === "info-panel"
        ? "新增合集链接"
        : "新增资源链接";
  linkDialogForm.elements.platform.value = link.platform || "";
  linkDialogForm.elements.url.value = link.url || "";
  linkDialogForm.elements.title.value = link.title || "";
  linkDialogDelete.hidden = index < 0;
  linkDialog.showModal();
};

const saveRecordingLinkDialog = () => {
  const { form: host, index, mode } = state.recordingLinkDialogContext;
  if (!host) {
    return;
  }
  const platform = compact(linkDialogForm.elements.platform.value);
  const url = compact(linkDialogForm.elements.url.value);
  const title = compact(linkDialogForm.elements.title.value);
  if (!platform || !url) {
  ...
  if (mode === "info-panel") {
    renderInfoPanelState(host, {
      text: host.elements.infoPanelText?.value,
      articleId: host.elements.infoPanelArticleId?.value,
...
const formatBatchLinkLines = (items) =>
  formatLines(items || [], (item) => [item?.platform || "", item?.url || "", item?.title || ""].join(" | "));
const parseBatchLinkLines = (value) =>
  parseLines(value, (line) => {
    const [platform, url, title = ""] = parsePipeLine(line);
    if (!platform || !url) {
      return null;
    }
    return { platform, url, title };
...
const batchDraftFieldConfigs = {
  ...
  recording: [
    ...
    { field: "links", label: "资源链接（platform | url | title）", multiline: true, rows: 4 },
  ],
};
...
const buildArticlePayload = () => ({
  ...
  slug: compact(articleForm.elements.slug.value),
    ...
    return `
      <section class="owner-inline-editor">
        ...
        <div class="owner-inline-editor__grid owner-inline-editor__grid--named">
          ...
          <label><span>国家</span><input data-inline-entity-field="country" value="${escapeHtml(entity.country || "")}" /></label>
...
const buildInlineEntityPayload = (entityType, container) => {
  ...
  if (["composer", "person"].includes(entityType)) {
    ...
    const payload = {
      ...
      country: compact(container.querySelector('[data-inline-entity-field="country"]')?.value || entity.country || ""),
...
const buildEntitySummaryHtml = (entity, options = {}) => {
  ...
  if ("workId" in entity && "title" in entity) {
    ...
    const rows = [
      ...
      ["资源链接", (entity.links || []).map((item) => item.platform || hostLabel(item.url)).filter(Boolean).join(" / ")],
      ["备注", entity.notes],
    ]
...
const buildImageCandidateCardHtml = (candidate, proposal, index) => `
  <div class="owner-proposal__image ${candidate.id === proposal.selectedImageCandidateId ? "is-selected" : ""}">
    <div class="owner-proposal__image-grid">
      <img src="${escapeHtml(candidate.src)}" alt="${escapeHtml(candidate.title || proposal.summary)}" />
      <div>
  ...
  if (field.path === "links") {
    ...
    return `
      <section class="owner-proposal__editor-field owner-proposal__editor-field--full owner-structured-links" data-recording-link-host>
        ...
        <p class="owner-proposal__hint">原值：${escapeHtml(formatValue(field.before))}</p>
      </section>`;
  }
  const multiline = proposalFieldUsesTextarea(field);
  const fieldValue = escapeHtml(formatProposalFieldInputValue(field));
  const oldValue = escapeHtml(formatValue(field.before));
              ...
              editableFields.length
                ? `<section class="owner-proposal__editor">
                    ...
                    <div class="owner-proposal__editor-grid">${editableFields
                      .map((field) => buildProposalFieldEditorHtml(effectiveProposal, field, options))
                      .join("")}</div>
                  </section>`
                : ""
            }
...
const renderJob = (job) => {
  ...
  if (!job) {
    ...
    automationJobCurrent.textContent = "当前无任务。";
  ...
  `;
  automationJobCurrent.innerHTML = job.currentItem
    ? `<strong>${escapeHtml(job.currentItem.label)}</strong><p>${escapeHtml(job.currentItem.description || `${automationCategoryLabels[job.currentItem.category]} 条目`)}</p>`
    : `<strong>当前无正在处理的条目</strong><p>点击下方任务方块可查看单条详细信息。</p>`;
...
const buildBatchDraftCardHtml = (entryType, entry, index, selected) => {
  ...
  const summary =
    entryType === "recording"
      ...
      : entity.nameLatin || entity.country || "";
  return `
    <button
      type="button"
...
const renderBatchSession = () => {
  ...
  batchSessionSummary.innerHTML = `
    ...
    <div>条目数量：版本 ${draftEntities.recordings.length}</div>
    ${
...
entityForms.forEach((form) => {
  ...
  form.addEventListener("click", (event) => {
    ...
    if (infoPanelLinkButton) {
      openRecordingLinkDialog(form, Number(infoPanelLinkButton.dataset.infoPanelLinkEdit || -1));
    }
  });
});
