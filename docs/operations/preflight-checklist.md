...
- 项目类型：`web`
- 技术栈：`nodejs, typescript, playwright`
- 推荐运行环境：`wsl`
- 推荐终端：`bash`
- 决策依据：项目更依赖 Linux 工具链与 POSIX 命令，WSL（Windows 子系统 Linux）通常更稳定。

## 2. Codex 启动前设置（用户手动）
- [ ] `Agent environment`（Agent 运行环境）设置为：`wsl`
- [ ] `Integrated terminal shell`（集成终端）设置为：`bash`
