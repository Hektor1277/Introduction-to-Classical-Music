...
- `displayLatinName` 用于网页端与索引中的短拉丁名显示。

### `person`

- 负责非作曲家人物与各类团体。
- 当前 `roles` 已支持多角色并允许 `composer` 并入，避免“作曲家”和“人物”重复建模。
- 常见角色包括：
  - `composer`
  - `conductor`
  - `soloist`
...
- 负责“某一个具体版本 / 演出 / 录音”。
- `title` 为规范版本标题，但属于 derived field（派生字段），不是权威事实原文。
...
- `displayName` 用于回写和保底显示，不可完全依赖 `personId` 临时生成。
- 允许同一版本出现多个 `soloist`、多个 `singer`、多个 `ensemble`、多个 `chorus`，以支持歌剧、清唱剧、芭蕾舞剧、合唱交响曲、室内乐临时组合等场景。

### 复数参与者兼容策略

- 不新增第二套复合结构，继续使用现有 `credits[]`。
- 多人物 / 多团体版本通过多条 credit 表达，不通过拼接字符串硬编码。
- owner 端录音表单允许新增 / 删除任意数量 `credit row`（署名行）。
...
只允许在 owner 跳转控件中展示一级直接关联，避免把维护工具变成图遍历器：
...
- owner 与 site 对同一字段的语义不一致。

以下属于数据清洗问题：

- 版本缺失关键 credit。
- 乐团条目仍把 alias（别名）保留为 canonical name（规范名）。
- 历史 `title` 写死错误信息。
- 人物 / 团体的脏 alias 污染自动检查结果。
- 占位实体（如 `person-item`、`-`、`未知`）仍被正式条目引用。

## 6. 后续清理原则

- 规则变更优先改共享模块，再改 owner / site。
- 个别条目异常优先按数据清洗处理，不在展示层打补丁。
- 新增录音体裁或新增显示规则时，先更新规则文档，再更新共享模块与测试。
