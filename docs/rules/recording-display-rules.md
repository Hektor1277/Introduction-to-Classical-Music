...
- `recording.workTypeHint`：输入层体裁提示，当前规范值为：
  - `orchestral`
  - `concerto`
  - `opera_vocal`
  - `chamber_solo`
  - `unknown`
- `presentation family`（展示家族）：展示层归一化结果，不直接写回数据，用于决定标题、副标题、搜索文本与推荐卡片布局：
  - `orchestral`
  - `concerto`
  - `opera`
  - `solo`
...
- 所有标题与副标题一律从 `credits[]`、`performanceDateText`、`venueText` 和所属作品体裁派生。
- 旧 `recording.title` 只作为回退值，不作为优先事实来源。
- 多参与者场景下必须保留顺序，不允许把多个参与者压成单个虚假团体。
...
- 当没有正式组合条目时，允许多位 `soloist` 直接进入标题。
- 副标题：对应原文或英文串联。
...
- credit 冲突或体裁信息不足时，优先退回 `orchestral` 家族或使用原始 `recording.title`。
- 指挥与独奏撞人时，展示层去重，避免同一人同时出现在多条人物线。
- 若结构化 credit 完整而历史标题明显不一致，优先信任结构化 credit，并将问题归类为数据清洗。

## 7. 变更原则

- 先更新本规则，再更新共享模块 `packages/shared/src/recording-rules.ts` 与 `packages/shared/src/display.ts`。
- 批量导入模板、owner 表单、网页端显示必须共用同一套体裁归一化与标题生成逻辑。
