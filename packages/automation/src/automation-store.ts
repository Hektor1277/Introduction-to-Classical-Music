...
const automationDir = path.join(rootDir, "data", "automation");
const runsDir = path.join(automationDir, "runs");
const settingsPath = path.join(automationDir, "settings.local.json");
const recordingRetrievalSettingsPath = path.join(automationDir, "recording-retrieval.local.json");
const assetsDir = path.join(rootDir, "apps", "site", "public", "library-assets", "managed");
...
export function getAutomationPaths() {
  return {
    ...
    runsDir,
    settingsPath,
    recordingRetrievalSettingsPath,
    assetsDir,
  };
}
...
export async function loadRecordingRetrievalConfig() {
  await ensureAutomationDirs();
  try {
    const content = await fs.readFile(recordingRetrievalSettingsPath, "utf8");
