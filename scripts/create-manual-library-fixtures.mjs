...
const variants = {
  "library-a.icmlibrary": base,
  "library-b.icmlibrary": { ...clone(base), works: [...clone(base).works, { ...clone(base).works[0], id: "work-b-only", slug: "work-b-only", title: "B 独有作品", sortKey: "b-only" }], recordings: [{ ...clone(base).recordings[0], title: "共享版本（B 修订）" }] },
};
for (const [name, library] of Object.entries(variants)) {
  ...
  await writeFile(path.join(dir, "library.manifest.json"), `${JSON.stringify({ schemaVersion: "library-bundle-v1", libraryId: name, libraryName: name.replace(".icmlibrary", ""), createdAt: "2026-09-12T00:00:00.000Z", updatedAt: "2026-09-12T00:00:00.000Z", appMinVersion: "0.1.0" }, null, 2)}\n`);
